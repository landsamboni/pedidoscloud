...
// Orders still needing action from the restaurant.
const PENDING_STATUSES = new Set(["NEW", "PAYMENT_PENDING", "PAYMENT_REVIEW"]);

function urgencyBorder(order: Order, now: number | null): string {
  if (!now || !PENDING_STATUSES.has(order.status)) return "";
...
interface Props {
  ...
  orders: Order[];
  readOnly?: boolean; // true in history view — disables refresh and action buttons
}

export function RestaurantOrders({ restaurantSlug, orders, readOnly = false }: Props) {
  const router = useRouter();
  // null on SSR — avoids hydration mismatch. Set on client mount.
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 xl:items-start">
      {orders.map((order) => {
        ...
        return (
          <article className={`card flex flex-col gap-0 ${border}`} key={order.id}>
            ...
            {order.paymentProofPath && (
              <div className="mt-4 rounded-xl bg-purple-50 p-3">
                ...
                <a className="button-secondary mt-2 inline-block border-purple-300 text-purple-800"
                   href={order.paymentProofPath} rel="noreferrer" target="_blank">
                  Ver comprobante
                </a>
              </div>
            )}
