...
export function PaymentProofForm({ compact = false, publicToken, restaurantSlug }: { compact?: boolean; publicToken: string; restaurantSlug: string }) {
  ...
  return (
    <form action={action} className={compact ? "mt-3 space-y-3" : "mt-4 space-y-3"}>
      ...
      <input accept="image/jpeg,image/png,image/webp,application/pdf" className="input text-base file:mr-3 file:rounded-lg file:border-0 file:bg-stone-200 file:px-3 file:py-2 file:font-semibold" name="paymentProof" required type="file" />
