...
export default async function AdminRestaurantPage({ params }: { params: Promise<{ restaurantSlug: string }> }) {
  ...
  const endDateLong = restaurant.subscriptionEndsAt
    ? new Date(restaurant.subscriptionEndsAt).toLocaleDateString("es-CO", { weekday: "long", day: "numeric", month: "long", year: "numeric" })
    : null;
  const waMessages: Record<typeof status, string> = {
    "no-subscription": `¡Hola ${restaurant.name}! 👋 Te damos la bienvenida a PedidosCloud. Para activar tu suscripción y empezar a recibir pedidos en línea, realiza tu pago y notifícanos. ¡Estamos aquí para ayudarte a crecer! 🚀`,
    "active": `¡Hola ${restaurant.name}! 😊 Todo está en orden con tu suscripción de PedidosCloud, que renueva el ${endDateLong}. Cuando llegue la fecha, recuerda hacer tu pago para seguir sin interrupciones. ¡Gracias por confiar en nosotros! 🙌`,
    "expiring-soon": `¡Hola ${restaurant.name}! ⏰ Te recordamos que tu suscripción de PedidosCloud vence el ${endDateLong}. Para seguir recibiendo pedidos sin interrupciones, realiza tu pago antes de esa fecha. ¡Contamos contigo! 💪`,
    "suspended": `¡Hola ${restaurant.name}! Tu cuenta en PedidosCloud está suspendida por falta de pago. Para reactivarla, realiza tu pago de renovación y avísanos. ¡Te esperamos de vuelta pronto! 😊`,
  };
  const waUrl = `https://wa.me/${waNumber}?text=${encodeURIComponent(waMessages[status])}`;
    ...
    <main className="mx-auto max-w-4xl p-4 sm:p-6 3xl:max-w-5xl">
      <header className="flex flex-wrap items-start justify-between gap-3 py-5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Link className="button-secondary" href={`/r/${restaurant.slug}`} rel="noreferrer" target="_blank">Página cliente ↗</Link>
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3">
          ...
          {daysLeft !== null && (
            <span className="text-sm text-stone-500">
              {daysLeft > 0 ? `${daysLeft} día${daysLeft !== 1 ? "s" : ""} restante${daysLeft !== 1 ? "s" : ""}` : daysLeft === 0 ? "Vence hoy" : `Venció hace ${Math.abs(daysLeft)} día${Math.abs(daysLeft) !== 1 ? "s" : ""}`}
            </span>
          )}
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-xl border border-stone-200 bg-stone-50 p-3">
            ...
            <form action={setRestaurantSubscription} className="mt-2 space-y-2">
              ...
              <input className="input text-sm" defaultValue={new Date().toISOString().slice(0, 10)} max={new Date().toISOString().slice(0, 10)} name="paymentDate" required type="date" />
              <button className="button-primary w-full text-sm" type="submit">{status === "no-subscription" ? "Activar" : "Registrar pago"}</button>
            </form>
          </div>
          <div className="rounded-xl border border-teal-200 bg-teal-50 p-3">
            ...
            <form action={setRestaurantSubscription} className="mt-2 space-y-2">
              ...
              <input className="input text-sm" defaultValue={defaultEndDateStr} min={new Date().toISOString().slice(0, 10)} name="endDate" required type="date" />
          ...
          <div className="rounded-xl border border-red-200 bg-red-50 p-3">
            ...
            <form action={deactivateRestaurantSubscription} className="mt-2">
              <input name="restaurantId" type="hidden" value={restaurant.id} />
              <button className="w-full rounded-xl border border-red-400 bg-white px-3 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-50" type="submit">Desactivar suscripción</button>
            </form>
          </div>
        </div>

        {waPhone && (
          <a className="mt-3 inline-flex items-center gap-2 rounded-xl bg-[#25D366] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#1ebe5d]" href={waUrl} rel="noreferrer" target="_blank">
      ...
      <details className="mt-4 rounded-xl border border-red-200 bg-red-50 p-4">
        <summary className="cursor-pointer text-sm font-semibold text-red-700">⚠ Zona peligrosa — eliminar órdenes / restaurante</summary>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div className="rounded-xl border border-red-300 bg-white p-4">
            ...
            <p className="mt-1 text-xs text-red-600">Borra todas las órdenes y contadores de este restaurante. No se puede deshacer.</p>
            <form action={deleteRestaurantOrders} className="mt-3 space-y-2">
              ...
              <button className="w-full rounded-xl border border-red-400 bg-white px-3 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-50" type="submit">Eliminar todo el historial</button>
            </form>
          </div>
          <div className="rounded-xl border border-red-300 bg-white p-4">
            <p className="text-sm font-semibold text-red-800">Eliminar órdenes de un día</p>
            <p className="mt-1 text-xs text-red-600">Borra solo las órdenes de la fecha seleccionada.</p>
            <form action={deleteRestaurantOrdersByDate} className="mt-3 space-y-2">
              <input name="restaurantId" type="hidden" value={restaurant.id} />
              <label className="block text-xs font-medium text-red-700">Fecha (YYYY-MM-DD):
                <input className="input mt-1 text-sm" name="date" required type="date" />
              </label>
              <button className="w-full rounded-xl border border-red-400 bg-white px-3 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-50" type="submit">Eliminar órdenes del día</button>
            </form>
          </div>
      ...
      {/* Today's orders */}
      <section className="card mt-4">
        <h2 className="font-semibold">Pedidos de hoy ({restaurant.orders.length})</h2>
        {restaurant.orders.length ? (
          <div className="mt-3 overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="text-stone-500"><tr><th className="pb-2">Orden</th><th>Cliente</th><th>Estado</th><th>Total</th></tr></thead>
              <tbody>
