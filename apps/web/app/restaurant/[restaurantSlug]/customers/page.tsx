  ...
  return (
    <main className="mx-auto max-w-4xl p-4 sm:p-6 2xl:max-w-6xl">
      <header className="mb-6 flex flex-wrap items-start justify-between gap-3 py-5">
        ...
        <Link className="button-secondary" href={`/restaurant/${restaurantSlug}`}>
          ← Consola
        </Link>
      </header>
            ...
            <details className="card group" key={c.id}>
              <summary className="flex cursor-pointer list-none items-center justify-between gap-3">
                ...
                <div className="hidden sm:flex items-center gap-6 shrink-0 text-right">
                  <div>
                    ...
                    <p className="font-bold text-stone-900">{c.confirmedOrders}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-xs text-stone-400">Promedio / pedido</p>
                ...
                <div className="flex sm:hidden flex-col items-end shrink-0 text-right gap-0.5">
                  <p className="font-bold text-brand-blue">{formatMoney(c.totalSpent)}</p>
              ...
              <div className="mt-4 border-t border-stone-100 pt-4 space-y-3">
                ...
                <div className="flex sm:hidden gap-4 text-sm">
                  <div>
                    <p className="text-xs text-stone-400">Promedio/pedido</p>
                    <p className="font-semibold">{formatMoney(c.avgOrder)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-stone-400">Última compra</p>
                    <p className="font-semibold">{c.lastOrderDate ? timeAgo(c.lastOrderDate) : "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-stone-400">Total pedidos</p>
                    <p className="font-semibold">{c.totalOrders}</p>
                  </div>
                </div>

                <div className="text-sm text-stone-600">
                  <span className="font-medium text-stone-700">Dirección:</span> {c.lastAddress}
                </div>
                ...
                {(() => {
                  ...
                  return (
                    <a
                      className="inline-flex items-center gap-2 rounded-xl bg-[#25D366] px-3 py-1.5 text-sm font-semibold text-white hover:bg-[#1ebe5d] transition"
