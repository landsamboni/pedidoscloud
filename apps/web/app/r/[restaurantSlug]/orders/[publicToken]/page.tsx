...
export default async function PublicOrderPage({ params }: { params: Promise<{ restaurantSlug: string; publicToken: string }> }) {
  ...
  const whatsAppUrl = buildWhatsAppUrl(order.restaurant.nequiPhone, order.restaurant.name, order.orderNumber);

  return (
    <main className="mx-auto max-w-2xl p-4 sm:p-6">
      <header className="py-5">
        <p className="text-sm font-bold uppercase tracking-wide text-violet-700">{order.restaurant.name}</p>
        ...
        <section className="card mt-4">
          ...
          {paymentConfigured ? (
            <>
              ...
              <div className="mt-4 rounded-2xl bg-violet-50 p-4 text-base">
                <p className="text-sm font-semibold uppercase tracking-wide text-violet-700">Celular o llave Nequi</p>
      ...
      {isWaiting && whatsAppUrl && (
        <section className="card mt-4 border-stone-200 bg-stone-50">
          ...
          <p className="mt-1 text-sm leading-relaxed text-stone-500">
            Si llevas un rato esperando, puedes preguntarle directamente al restaurante por el estado de tu pedido.
          </p>
          <a
      ...
      <section className="mt-5">
        <Link className="block rounded-xl bg-violet-700 px-4 py-3 text-center text-base font-bold text-white shadow-sm transition hover:bg-violet-800" href={`/r/${restaurantSlug}`}>
          Hacer otro pedido
        </Link>
      </section>
    </main>
