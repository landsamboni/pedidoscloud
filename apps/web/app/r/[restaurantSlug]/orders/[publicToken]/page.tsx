...
export default async function PublicOrderPage({ params }: { params: Promise<{ restaurantSlug: string; publicToken: string }> }) {
  ...
  // Once the proof is submitted (or order is in a terminal state), show a clean
  // focused screen — no payment details noise, just the status front and center.
  const showCleanStatus = isWaiting || isConfirmed || isCancelled ||
    (order.status === "PAYMENT_REJECTED" && !!order.paymentProofPath);
  ...
  // ── Clean focused screen (proof sent / terminal state) ──────────────────
  if (showCleanStatus) {
    const bgClass = isConfirmed ? "bg-emerald-50" : isCancelled ? "bg-red-50" : "bg-amber-50";
    const iconBg = isConfirmed ? "bg-emerald-500" : isCancelled ? "bg-red-500" : "bg-amber-500";
    const icon = isConfirmed ? "✓" : isCancelled ? "✕" : "…";
    const titleColor = isConfirmed ? "text-emerald-900" : isCancelled ? "text-red-900" : "text-amber-900";
    const bodyColor = isConfirmed ? "text-emerald-800" : isCancelled ? "text-red-800" : "text-amber-800";
    const title = isConfirmed
      ? "¡Tu pago fue confirmado!"
      : isCancelled
      ? "Pedido cancelado"
      : order.status === "PAYMENT_REJECTED"
      ? "Debes corregir tu comprobante"
      : "Comprobante enviado";
    const body = isConfirmed
      ? "El restaurante verificó tu pago y está preparando tu pedido. ¡Gracias!"
      : isCancelled
      ? "El restaurante canceló este pedido. Contáctalo para saber el motivo."
      : order.status === "PAYMENT_REJECTED"
      ? "El restaurante no pudo validar el archivo anterior. Reemplázalo por el comprobante correcto."
      : "El restaurante recibió tu comprobante y lo está revisando. Esta página se actualiza sola — no la cierres.";

    const waUrl = isConfirmed ? assistanceWhatsAppUrl : isCancelled ? cancelWhatsAppUrl : whatsAppUrl;

    return (
      <main className={`min-h-screen ${bgClass} flex flex-col items-center justify-start p-4 pt-16 sm:pt-24`}>
        ...
        {isWaiting && (
          <div className="relative mb-6 flex h-20 w-20 items-center justify-center">
            ...
            <span className="relative inline-flex h-14 w-14 items-center justify-center rounded-full bg-amber-500 text-3xl font-bold text-white">⏳</span>
          </div>
        )}
        {!isWaiting && (
          <div className={`mb-6 flex h-20 w-20 items-center justify-center rounded-full text-3xl font-bold text-white shadow-lg ${iconBg}`}>
            {icon}
          </div>
        )}

        <h1 className={`text-3xl font-black tracking-tight ${titleColor} text-center`}>{title}</h1>
        <p className={`mt-3 max-w-sm text-center text-base leading-relaxed ${bodyColor}`}>{body}</p>

        <div className={`mt-4 rounded-2xl px-5 py-3 text-center text-sm font-semibold ${titleColor} bg-white/60`}>
          {order.restaurant.name} · Pedido {formatOrderNumber(order.orderNumber)} · {formatMoney(Number(order.total))}
        </div>

        {/* WhatsApp button */}
        {waUrl && (
          <a
            className="mt-6 inline-flex items-center gap-2 rounded-xl bg-[#25D366] px-5 py-3 text-base font-semibold text-white shadow transition hover:bg-[#1ebe5d]"
            href={waUrl}
            rel="noreferrer"
            target="_blank"
          >
            ...
            {isConfirmed ? "Contactar al restaurante" : isCancelled ? "Preguntar al restaurante" : "Preguntar por WhatsApp"}
          </a>
        )}

        {/* Replace proof (rejected) */}
        {order.status === "PAYMENT_REJECTED" && (
          <div className="mt-6 w-full max-w-sm rounded-2xl bg-white p-5 shadow">
            <p className="mb-3 text-sm font-semibold text-stone-700">Reemplazar comprobante</p>
            <PaymentProofForm compact publicToken={publicToken} restaurantSlug={restaurantSlug} />
          </div>
        )}

        <Link className="mt-8 text-sm font-medium text-stone-500 underline hover:text-stone-700" href={`/r/${restaurantSlug}`}>
          Hacer otro pedido
        </Link>
      </main>
    );
  ...
  // ── Normal screen (pending payment — proof not yet sent) ─────────────────
  return (
    <main className="mx-auto max-w-2xl p-4 sm:p-6">
      ...
      {/* These states are handled by the clean screen above — dead code guard */}
      {isCancelled && (
